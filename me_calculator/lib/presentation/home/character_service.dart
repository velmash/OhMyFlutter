...
class CharacterService {
  ...
  //   try {
  //     DocumentSnapshot doc = await _firestore.collection('users').doc(userId).collection('characters').doc(ocid).get();
  ...
  //   return FirebaseFirestore.instance.collection('users').doc(userId).collection('characters').snapshots().map((
  //     snapshot,
  //   ) {
  //     print('Firestore 스냅샷 받음: ${snapshot.docs.length}개 문서'); // 디버그용
  //     return snapshot.docs.map((doc) => MyCharacter.fromMap(doc.data())).toList();
  //   });
  // }
  ...
  //     return querySnapshot.docs.map((doc) => MyCharacter.fromMap(doc.data() as Map<String, dynamic>)).toList();
  //   } catch (e) {
  //     print('캐릭터 조회 실패: $e');
  //     return [];
  //   }
